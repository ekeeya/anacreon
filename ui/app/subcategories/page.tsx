...
export default function SubCategoriesPage() {
  ...
  const [form, setForm] = useState<Partial<SubCategoryDto>>({});
  ...
  async function load() {
    try {
      const data = await api.get<SubCategoryDto[]>("/subcategories");
      setRows(data);
    } catch (e) {
      console.error(e);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable columns={columns} data={rows} emptyText="No subcategories yet" />

      <Modal title="New Subcategory" isOpen={open} onClose={() => setOpen(false)}>
